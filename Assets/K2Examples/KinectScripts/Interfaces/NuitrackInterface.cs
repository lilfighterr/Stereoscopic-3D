	                           ...
	                           ref Matrix4x4 kinectToWorld, bool bIgnoreJointZ)
	{
		...
		if(skeletonData != null && skeletonDataTimestamp != lastSkeletonFrameTimestamp)
		{
			...
			for(int i = 0; i < sensorData.bodyCount; i++)
			{
				...
				bodyFrame.bodyData[i].liTrackingID = (long)nuiBody.ID;
				...
				if (handTrackerData != null) 
				{
					...
					if (hands != null) 
					{
						...
						bodyFrame.bodyData[i].leftHandState = leftHand.HasValue && leftHand.Value.Click ? KinectInterop.HandState.Closed : KinectInterop.HandState.Open;
						bodyFrame.bodyData[i].leftHandConfidence = leftHand.HasValue ? KinectInterop.TrackingConfidence.High : KinectInterop.TrackingConfidence.Low;
					
						nuitrack.HandContent? rightHand = !dontHFlipBodyFrame ? hands.RightHand : hands.LeftHand;
						bodyFrame.bodyData[i].rightHandState = rightHand.HasValue && rightHand.Value.Click ? KinectInterop.HandState.Closed : KinectInterop.HandState.Open;
						bodyFrame.bodyData[i].rightHandConfidence = rightHand.HasValue ? KinectInterop.TrackingConfidence.High : KinectInterop.TrackingConfidence.Low;
					}
				}
